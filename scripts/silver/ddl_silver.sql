...
Note:
    - The silver tables must already exist, and the bronze layer must be
      loaded first (EXEC bronze.load_bronze).
===============================================================================
*/
...
BEGIN
    ...
    BEGIN TRY
        ...
        END cst_gender,
        cst_create_date
        -- Deduplicate: number the rows of each customer by create date and
        -- keep only the first one (flag_last = 1)
        FROM (SELECT *,
       ...
       PRINT '-----------------------------------------------------';
        ...
        subcat,
        maintenance
        FROM bronze.erp_px_cat_g1v2 WHERE subcat!=TRIM(subcat);
        SET @end_time = GETDATE();
